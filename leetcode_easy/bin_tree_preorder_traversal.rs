...
fn preorder_traversal(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
    let mut log = vec![];
    preorder_traversal_inner(&root, &mut log);
    log
}

fn preorder_traversal_inner(root: &Option<Rc<RefCell<TreeNode>>>, log: &mut Vec<i32>) {
    if let Some(node) = root {
        let _node = node.borrow();
        log.push(_node.val);
        match _node.left {
            Some(ref subnode) => {
                preorder_traversal_inner(&Some(Rc::clone(subnode)), log);
            }
            None => {}
        };
        match _node.right {
